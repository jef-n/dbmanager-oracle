...
class OracleDBConnector(DBConnector):
	def __init__(self, uri):
		...
		try:
                        self.connection = cx_Oracle.connect( self.user, self.passwd, self.dbname) 
	...
	def _checkSpatial(self):
		...
		c = self._execute(None, u"SELECT 1 FROM v$option WHERE parameter='Spatial' AND value='TRUE'")
	...
	def _checkGeometryColumnsTable(self):
		...
		c = self._execute(None, u"SELECT PRIVILEGE FROM ALL_TAB_PRIVS_RECD WHERE TABLE_NAME IN ('ALL_SDO_GEOM_METADATA', 'USER_SDO_GEOM_METADATA') AND PRIVILEGE = 'SELECT' AND ROWNUM=1")
	...
	def getSchemaPrivileges(self, schema):
		...
		schema = self.user if schema == None else schema
                # In Oracle world, rights seems quite simple: only schema_owner can create table in the schema
	...
	def getTablePrivileges(self, table):
		...
		schema, tablename = self.getSchemaTableName(table)
                ...
                sql = u"SELECT DISTINCT PRIVILEGE FROM ALL_TAB_PRIVS WHERE privilege IN ('SELECT','INSERT','UPDATE','DELETE') AND TABLE_NAME = %s %s" % (self.quoteString(tablename), schema_where)
		...
		res = self._fetchall(c)
                ...
                for line in res:
                        if line[0] == u"SELECT": result[0] = True
                        if line[0] == u"INSERT": result[1] = True
                        if line[0] == u"UPDATE": result[2] = True
                        if line[0] == u"DELETE": result[3] = True
	...
	def getTables(self, schema=None):
		...
		# get all non geographic tables and views
			...
			schema_where = u"WHERE c.owner = %s AND o.owner = c.owner AND e.owner = c.owner " % self.quoteString(schema)
	...
	def getVectorTables(self, schema=None):
		...
		# discovery of all geographic tables
                ...
                if self.userTablesOnly:
                        ...
                        owner = u"user As owner"
                if schema and not self.userTablesOnly:
			schema_where = u" WHERE c.owner = %s AND o.owner = c.owner AND d.owner = c.owner AND e.owner = c.owner " % self.quoteString(schema)
		...
		try:
			...
			return QGis.WKBUnknown
                ...
                for row in rows:
                        if row[0] == 2001: geom_types['Point'] = row[1]
                        elif row[0] == 2002: geom_types['Linestring'] = row[1]
                        elif row[0] == 2003: geom_types['Polygon'] = row[1]
                        elif row[0] == 2005: geom_types['MultiPoint'] = row[1]
                        elif row[0] == 2006: geom_types['MultiLinestring'] = row[1]
                        elif row[0] == 2007: geom_types['MultiPolygon'] = row[1]

                # Make the decision:
                champion = list(sorted(geom_types, key=geom_types.__getitem__, reverse=True))[0]
                if champion == u"Point":
                        wkbType = QGis.WKBPoint
                        if geom_types['MultiPoint'] > 0: wkbType = QGis.WKBMultiPoint
                elif champion == u"MultiPoint":
                        wkbType = QGis.WKBMultiPoint
                elif champion == u"Linestring":
                        wkbType = QGis.WKBLineString
                        if geom_types['MultiLinestring'] > 0: wkbType = QGis.WKBMultiLineString
                elif champion == u"MultiLinestring":
                        wkbType = QGis.WKBMultiLineString
                elif champion == u"Polygon":
                        wkbType = QGis.WKBPolygon
                        if geom_types['MultiPolygon'] > 0: wkbType = QGis.WKBMultiPolygon
	...
	def getTableFields(self, table):
		...
		schema, tablename = self.getSchemaTableName(table)
		schema_where = u" AND a.OWNER=%s " % self.quoteString(schema) if schema is not None else ""
	...
	def getTableIndexes(self, table):
		...
		schema, tablename = self.getSchemaTableName(table)
		schema_where = u" AND i.OWNER=%s " % self.quoteString(schema) if schema is not None else ""
	...
	def getTableConstraints(self, table):
		schema, tablename = self.getSchemaTableName(table)
		schema_where = u" AND c.OWNER=%s " % self.quoteString(schema) if schema is not None else ""
	...
	def enableTableTrigger(self, trigger, enable, table):
		...
		sql = u"ALTER TRIGGER %s %s" % (trigger, "ENABLE" if enable else "DISABLE")
	...
	def getTableEstimatedExtent(self, table, geom):
		...
		c.close()
                ...
                sql = u"SELECT sdo_lb,sdo_ub FROM mdsys.all_sdo_geom_metadata m, table(m.diminfo) WHERE owner=%s AND table_name=%s AND column_name=%s AND sdo_dimname='Y'" %(self.quoteString(schema), self.quoteString(tablename), self.quoteString(geom))
	...
	def getViewDefinition(self, view):
		...
		schema_where = u" AND OWNER=%s " % self.quoteString(schema) if schema is not None else ""

		sql = u"""SELECT TEXT FROM ALL_VIEWS WHERE VIEW_NAME = %s %s""" % (self.quoteString(tablename), schema_where)
	...
	def getSpatialRefInfo(self, srid):
		...
		try:
			c = self._execute(None, "SELECT CS_NAME FROM MDSYS.CS_SRS WHERE SRID = %d" % srid)
		except DbError, e:
			return
		sr = self._fetchone(c)
		c.close()
	...
	def isVectorTable(self, table):
		...
		if self.has_geometry_columns and self.has_geometry_columns_access:
			schema, tablename = self.getSchemaTableName(table)
			sql = u"SELECT count(*) FROM all_sdo_geom_metadata WHERE owner = %s AND table_name = %s" % (self.quoteString(schema), self.quoteString(tablename))
	...
	def deleteTable(self, table):
		...
		schema, tablename = self.getSchemaTableName(table)
		schema_part = u"AND owner = %s " % self.quoteString(schema) if schema is not None else ""
                
		if self.isVectorTable(table):
                        if self.getTablePrivileges('ALL_SDO_GEOM_METADATA')[3]:
                                sql = u"DELETE FROM ALL_SDO_GEOM_METADATA WHERE table_name = %s %s" % (self.quoteString(tablename), schema_part)
                        elif schema.lower() == self.user.lower() and self.getTablePrivileges('USER_SDO_GEOM_METADATA')[3]:
                                sql = u"DELETE FROM USER_SDO_GEOM_METADATA WHERE table_name = %s" % self.quoteString(tablename)
                        self._execute_and_commit(sql)
	...
	def renameTable(self, table, new_table):
		...
		if self.isVectorTable(table):
                        if self.getTablePrivileges('ALL_SDO_GEOM_METADATA')[2]:
                                sql = u"UPDATE ALL_SDO_GEOM_METADATA SET TABLE_NAME = %s WHERE table_name = %s AND owner = %s" % (self.quoteString(new_table), self.quoteString(tablename), self.quoteString(schema))
                        elif schema.lower() == self.user.lower() and self.getTablePrivileges('USER_SDO_GEOM_METADATA')[2]:
                                sql = u"UPDATE USER_SDO_GEOM_METADATA SET TABLE_NAME = %s WHERE table_name = %s" % (self.quoteString(new_table), self.quoteString(tablename))
                        else:
                                self._commit()
	...
	def deleteTableColumn(self, table, column):
		...
		sql = u"ALTER TABLE %s DROP COLUMN %s" % (self.quoteId(table), self.quoteId(column))
		self._execute_and_commit(sql)
	...
	def updateTableColumn(self, table, column, new_name=None, data_type=None, not_null=None, default=None):
		...
		if new_name != None and new_name != column:
			sql = u"ALTER TABLE %s RENAME COLUMN %s TO %s" % (self.quoteId(table), self.quoteId(column), self.quoteId(new_name))
			self._execute(c, sql)
	...
	def isGeometryColumn(self, table, column):
		...
		sql = u"SELECT count(*) FROM all_sdo_geom_metadata WHERE table_name=%s AND column_name = %s %s" % (self.quoteString(tablename), self.quoteString(column.upper()), schema_where)
	...
	def addGeometryColumn(self, table, geom_column='GEOM', geom_type='POINT', srid=-1, dim=2):
		...
		schema, tablename = self.getSchemaTableName(table)
                ...
                sql = u"ALTER TABLE %s ADD %s SDO_GEOMETRY" % (self.quoteId(table) , self.quoteId(geom_column))
	...
	def createSpatialIndex(self, table, geom_column='GEOM'):
		...
		sql = u"CREATE INDEX %s ON %s(%s) INDEXTYPE IS MDSYS.SPATIAL_INDEX" % (idx_name, self.quoteId(table), self.quoteId(geom_column))
		self._execute_and_commit(sql)
	...
	def execution_error_types(self):
		return cx_Oracle.Error, cx_Oracle.ProgrammingError, cx_Oracle.Warning

	def connection_error_types(self):
		return cx_Oracle.InterfaceError, cx_Oracle.OperationalError
