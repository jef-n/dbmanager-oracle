    ...
    def spatialInfo(self):
        ...
        tbl = [
            (QApplication.translate(
             "DBManagerPlugin", "Oracle Spatial:"), info[0])
        ]
...
class ORTableInfo(TableInfo):
    ...
    def generalInfo(self):
        ...
        tbl = [
            (QApplication.translate("DBManagerPlugin", "Relation type:"), QApplication.translate(
             "DBManagerPlugin", "View") if self.table.isView else QApplication.translate("DBManagerPlugin", "Table")),
                (QApplication.translate(
                    "DBManagerPlugin", "Owner:"), self.table.owner)
        ]
        if self.table.comment:
            tbl.append(
                (QApplication.translate("DBManagerPlugin", "Comment:"), self.table.comment))
        ...
        if schema_priv == None:
            ...
            tbl.append((QApplication.translate("DBManagerPlugin", "Privileges:"), QApplication.translate(
                "DBManagerPlugin", "<warning> This user doesn't have usage privileges for this schema!")))
        else:
            ...
            if table_priv[0]:
                ...
                if self.table.rowCount == None or self.table.rowCount >= 0:
                    tbl.append((QApplication.translate("DBManagerPlugin", "Rows (counted):"), self.table.rowCount if self.table.rowCount != None else QApplication.translate(
                        "DBManagerPlugin", 'Unknown (<a href="action:rows/recount">find out</a>)')))
            ...
            priv_string = u", ".join(privileges) if len(privileges) > 0 else QApplication.translate(
        ...
        if schema_priv != None and schema_priv[1]:
            if table_priv[0] and not table_priv[1] and not table_priv[2] and not table_priv[3]:
                ret.append(
                    HtmlParagraph(QApplication.translate("DBManagerPlugin", "<warning> This user has read-only privileges.")))

        # primary key defined?
        if not self.table.isView:
            if len(filter(lambda fld: fld.primaryKey, self.table.fields())) <= 0:
                ret.append(
                    HtmlParagraph(QApplication.translate("DBManagerPlugin", "<warning> No primary key defined for this table!")))
    ...
    def fieldsDetails(self):
        ...
        header = (
            "#", QApplication.translate("DBManagerPlugin", "Name"), QApplication.translate(
                "DBManagerPlugin", "Type"), QApplication.translate("DBManagerPlugin", "Length"),
                  QApplication.translate("DBManagerPlugin", "Null"), QApplication.translate("DBManagerPlugin", "Default"), QApplication.translate("DBManagerPlugin", "Comment"))
        tbl.append(HtmlTableHeader(header))

        # add table contents
        for fld in self.table.fields():
            char_max_len = fld.charMaxLen if fld.charMaxLen != None and fld.charMaxLen != - \
                1 else ""
            ...
            tbl.append(
                (fld.num, name, fld.type2String(), char_max_len, is_null_txt, fld.default2String(), fld.comment))
    ...
    def constraintsDetails(self):
        ...
        # define the table header
        header = (QApplication.translate("DBManagerPlugin", "Name"), QApplication.translate(
            "DBManagerPlugin", "Type"), QApplication.translate("DBManagerPlugin", "Column(s)"))
        tbl.append(HtmlTableHeader(header))
    ...
    def triggersDetails(self):
        ...
        for trig in self.table.triggers():
            ...
            (enabled, action) = (QApplication.translate("DBManagerPlugin", "Yes"),
                                 "disable") if trig.enabled == u"ENABLED" else (QApplication.translate("DBManagerPlugin", "No"), "enable")
        ...
        ret.append(
            HtmlParagraph(QApplication.translate("DBManagerPlugin", '<a href="action:triggers/enable">Enable all triggers</a> / <a href="action:triggers/disable">Disable all triggers</a>')))
    ...
    def getTableInfo(self):
        ...
        if general_info == None:
            ...
            ret.append(
                HtmlSection(QApplication.translate("DBManagerPlugin", 'General info'), general_info))
        ...
        if spatial_info == None:
            ...
            ret.append(
                HtmlSection(self.table.database().connection().typeNameString(), spatial_info))
        ...
        if fields_details == None:
            ...
            ret.append(
                HtmlSection(QApplication.translate("DBManagerPlugin", 'Fields'), fields_details))
        ...
        if constraints_details == None:
            ...
            ret.append(
                HtmlSection(QApplication.translate("DBManagerPlugin", 'Constraints'), constraints_details))
        ...
        if indexes_details == None:
            ...
            ret.append(
                HtmlSection(QApplication.translate("DBManagerPlugin", 'Indexes'), indexes_details))
        ...
        if triggers_details == None:
            ...
            ret.append(
                HtmlSection(QApplication.translate("DBManagerPlugin", 'Triggers'), triggers_details))
    ...
    def spatialInfo(self):
            ...
            (QApplication.translate("DBManagerPlugin", "Column:"),
             self.table.geomColumn),
                ...
                (QApplication.translate("DBManagerPlugin",
                 "QGis Geometry type:"), self.table.wkbType)
        ]

        # only if we have info from geometry_columns
        if self.table.geomDim:
            tbl.append(
                (QApplication.translate("DBManagerPlugin", "Dimension:"), self.table.geomDim))

        srid = self.table.srid if self.table.srid != None else -1
        sr_info = self.table.database().connector.getSpatialRefInfo(
            srid) if srid != -1 else QApplication.translate("DBManagerPlugin", "Undefined")
        if sr_info:
            tbl.append(
                (QApplication.translate("DBManagerPlugin", "Spatial ref:"), u"%s (%d)" % (sr_info, srid)))
        ...
        if not self.table.isView:
            ...
            if self.table.estimatedExtent != None and self.table.estimatedExtent[0] != None:
                ...
                tbl.append(
                    (QApplication.translate("DBManagerPlugin", "Estimated extent:"), estimated_extent_str))
        ...
        if self.table.extent != None and self.table.extent[0] != None:
            ...
            extent_str = QApplication.translate(
                "DBManagerPlugin", '(unknown) (<a href="action:extent/get">find out</a>)')
        tbl.append(
        ...
        if self.table.extent != None and self.table.extent[0] != None and self.table.estimatedExtent != None and self.table.estimatedExtent[0] != None and self.table.extent != self.table.estimatedExtent:
            ret.append(
                HtmlParagraph(QApplication.translate("DBManagerPlugin", '<warning> Metadata extent is different from real extent. You should <a href="action:extent/update">update it</a> !')))

        # is there an entry in geometry_columns?
        if self.table.geomType.lower() == 'geometry':
            ret.append(
                HtmlParagraph(QApplication.translate("DBManagerPlugin", "<warning> There isn't entry in geometry_columns!")))

        # find out whether the geometry column has spatial index on it
        if not self.table.isView:
            if not self.table.hasSpatialIndex():
                ret.append(
                    HtmlParagraph(QApplication.translate("DBManagerPlugin", '<warning> No spatial index defined (<a href="action:spatialindex/create">create it</a>)')))
